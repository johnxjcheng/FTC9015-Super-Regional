...
//
    ...
    private String configFileName = RobotInfo.CONGIG_FILENAME;

    public FtcConfig() {
        param = new Param();
    }

    public void set_color(boolean red, Context context, OpMode opMode) {
        // setup initial configuration parameters here

        opMode.telemetry.addData("set_color", "Red="+red);
        if (red==true) {
            opMode.telemetry.addData("set_color", "RED");
            param.delayInSec = 0;
            param.whiteTape = 190;
            param.position = 1;
        ...
        {
            opMode.telemetry.addData("set_color", "BLUE");
        ...
        try {
            ...
            outputStreamWriter.write(Integer.toString(param.whiteTape)+"\n");
            outputStreamWriter.write(Integer.toString(param.position)+"\n");
    ...
    public void init(Context context, OpMode opMode) {
        ...
        try {
            ...
            if ( inputStream != null ) {
                ...
                param.whiteTape  = Integer.valueOf(bufferedReader.readLine());
                param.position   = Integer.valueOf(bufferedReader.readLine());
    ...
    public void init_loop(Context context, OpMode opMode) {
        ...
        currConfigStepCheck = ConfigStep.WHITETAPE;
        // message to driver about state of this config parameter
        if (configStepState.ordinal() >= currConfigStepCheck.ordinal()) {
            opMode.telemetry.addData("C" + currConfigStepCheck.ordinal(), "WhiteValue: " + param.whiteTape );
        }
        // configure this parameter
        if (configStepState == currConfigStepCheck) {
            opMode.telemetry.addData("C" + configStepState.ordinal() + "A", "Push Y for +, A for -");
            ...
            if (a1 && !lastA1) {
                param.whiteTape--;
                if (param.whiteTape < 0) {
                    param.whiteTape = 0;
                }
            }
        }

        currConfigStepCheck = ConfigStep.POSITION;
        // message to driver about state of this config parameter
        if (configStepState.ordinal() >= currConfigStepCheck.ordinal()) {
            opMode.telemetry.addData("C" + currConfigStepCheck.ordinal(), "robotposition: " + param.position );
        }
        // configure this parameter
        if (configStepState == currConfigStepCheck) {
            opMode.telemetry.addData("C" + configStepState.ordinal() + "A", "Push Y for +, A for -");
